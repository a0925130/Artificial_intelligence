...
for train_idx, test_idx in kf.split(data_x):
    ...
    np.savez('TL_data' + str(count_kfold), history=his.history, predict=predict, test_data=test_y)
    plot_history(his, 'Tl' + str(count_kfold))
    plot_pred(test_y, predict, 'Tl' + str(count_kfold))
    plot_residuals(test_y, predict, 'Tl' + str(count_kfold))
    count_kfold += 1
